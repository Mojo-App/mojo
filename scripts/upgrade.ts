...
async function main() {
  ...
  const mojoCore = await upgrades.upgradeProxy(proxy, MojoCore, {
    kind: "uups",
  });
  mojoCore.deployed();
  assert(mojoCore.address === proxy, "proxy address changed");

  // Check new implementation
  const impl2 = await upgrades.erc1967.getImplementationAddress(mojoCore.address);
